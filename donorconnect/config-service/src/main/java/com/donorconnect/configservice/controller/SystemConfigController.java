...
@RestController @RequestMapping("/configs") @RequiredArgsConstructor
public class SystemConfigController {
    private final SystemConfigService service;
    @GetMapping public ResponseEntity<List<SystemConfig>> all() { return ResponseEntity.ok(service.getAll()); }
    @GetMapping("/{key}") public ResponseEntity<SystemConfig> byKey(@PathVariable String key) {
        return service.getByKey(key).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    @PostMapping public ResponseEntity<SystemConfig> save(@RequestBody SystemConfig config) { return ResponseEntity.ok(service.save(config)); }
